...
def _(pathlib, pl):
    ann_path = pathlib.Path(
        "/fs/scratch/PAS2136/cain429/beetlepalooza/annotations.json"
    )
    ...
    measurements_per_annoator = measurements_per_annoator.filter(
        pl.col("IsaFluck").is_not_null()
        & pl.col("rileywolcheski").is_not_null()
        & pl.col("ishachinniah").is_not_null()
    )
...
def _(gaussian_kde, measurements_per_annoator, np, plt):
    ...
    label_map = {
        "IsaFluck": fluck_data,
        "ishachinniah": isha_data,
        "rileywolcheski": riley_data,
    }
    comparison = [
        ["IsaFluck", "ishachinniah"],
        ["IsaFluck", "rileywolcheski"],
        ["ishachinniah", "rileywolcheski"],
    ]
    ...
    for ax, comparison in zip(axes, comparison):
        ...
        scatter = ax.scatter(
            x, y, c=z, s=15, cmap="viridis", alpha=0.8, edgecolor="none"
        )  # 'c=z' maps density to color
...
def _(fluck_data, isha_data, np, pl, riley_data):
    ...
    data = [
        get_metrics(fluck_data, isha_data, "IshaFluck vs ishachinniah"),
        get_metrics(fluck_data, riley_data, "IshaFluck vs rileywolcheski"),
        get_metrics(isha_data, riley_data, "ishachinniah vs rileywolcheski"),
    ]
