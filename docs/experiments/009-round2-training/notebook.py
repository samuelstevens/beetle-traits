...
def _(mo):
    mo.md("""
    # Experiment 009: Round 2 Training

    Single training run (LR=0.01, seed=17, WSD schedule, 60k steps) on Hawaii + Beetlepalooza + BioRepo labeled (train split). This is the first model trained with round-1 active-learning annotations from BioRepo. Inference over Hawaii (all splits) + BioRepo (all splits), plus ranking of unlabeled BioRepo for a potential round 2 annotation batch.

    **Goal metric**: percent error = |pred_length - gt_length| / gt_length * 100. Target: consistently below 0.3% (collaborator's CV^2 threshold).
    """)
    return
...
def _(pathlib, pl):
    ...
    labeled_df = pl.read_parquet(
        results_dpath / f"{run_id}_labeled.parquet"
    ).with_columns(
    ...
    return labeled_df, results_dpath, run_id


@app.cell
def _(pl):
    groupsDf = pl.read_csv(r"/fs/scratch/PAS2136/cain429/active_learning_round1/results/round2_groups.csv")
    beetlesDf = pl.read_csv(r"/fs/scratch/PAS2136/cain429/active_learning_round1/results/round2_beetles.csv")
    return (groupsDf,)
...
def _(mo):
    mo.md("""
    ## Percent error distribution by dataset
    """)
    return


@app.cell
def _(alt, labeled_df, pl):
    pct_df = (
        labeled_df
        .select("dataset", "length_pct_err", "width_pct_err")
        .unpivot(
        ...
        .with_columns(pl.col("metric").str.replace("_pct_err", ""))
    )
...
def _(mo):
    mo.md("""
    ## Absolute error distribution by dataset
    """)
    return
...
def _(alt, labeled_df, pl):
    err_df = (
        labeled_df
        .select("dataset", "width_line_err_cm", "length_line_err_cm")
        .unpivot(
        ...
        .with_columns(pl.col("metric").str.replace("_line_err_cm", ""))
    )
...
@app.cell
def _(alt, labeled_df):
    alt.Chart(
        labeled_df.select("mean_entropy", "length_pct_err", "dataset")
    ).mark_circle(size=10, opacity=0.3).encode(
...
@app.cell
def _(labeled_df, mo, np, pl):
    finite = labeled_df.filter(pl.col("length_pct_err").is_finite())
    n_total = len(finite)
    n_below = finite.filter(pl.col("length_pct_err") < 0.3).height
    pct_below = n_below / n_total * 100 if n_total > 0 else 0

    gt = np.array(labeled_df["gt_coords_px"].to_list())
...
@app.cell
def _(labeled_df, np, pl):
    _parts = []
    for _ds in labeled_df["dataset"].unique().to_list():
        _sub = labeled_df.filter(pl.col("dataset") == _ds)
...
def _(mo):
    ...
    2 Hawaii + 2 BioRepo samples at each error quantile (0%, 25%, 50%, 75%, 100%). Cyan/magenta = GT width/length, lime/yellow = predicted width/length.
    """)
    return
...
@app.cell
def _(Image, labeled_df, np, pl, plt):
    _base = labeled_df.filter(pl.col("length_pct_err").is_finite())
...
@app.cell
def _(pl, results_dpath, run_id):
    unlabeled_df = pl.read_parquet(results_dpath / f"{run_id}_unlabeled.parquet")
    return (unlabeled_df,)
...
def _(mo, unlabeled_df):
    ...
    **{unlabeled_df.height:,}** total unlabeled beetles.
    """)
    return


@app.cell
def _(alt, labeled_df, pl, unlabeled_df):
    _labeled = labeled_df.select("mean_entropy", "dataset").with_columns(
        pl.lit("labeled").alias("source")
    )
    _unlabeled = (
        unlabeled_df
        .select("mean_entropy", "dataset")
        .sample(n=min(5000, unlabeled_df.height), seed=0)
        .with_columns(pl.lit("unlabeled").alias("source"))
    )
    _ent_df = pl.concat([_labeled, _unlabeled])
    ...
    alt.Chart(_ent_df).mark_boxplot(extent="min-max").encode(
        ...
        color="source:N",
    ).properties(width=300, height=300, title="Entropy: labeled vs unlabeled")
    return


@app.cell
def _(alt, labeled_df, pl, unlabeled_df):
    _labeled_ent = labeled_df.select("mean_entropy").with_columns(
        pl.lit("labeled").alias("source")
    )
    _unlabeled_ent = (
        unlabeled_df
    ...
    alt.Chart(_hist_df).mark_bar(opacity=0.6).encode(
        ...
        color="source:N",
    ).properties(width=600, height=300, title="Entropy histogram")
    return


@app.cell
def _(mo, np, unlabeled_df):
    _ent = unlabeled_df["mean_entropy"].to_numpy()
    _q75 = np.percentile(_ent, 75)
    _q90 = np.percentile(_ent, 90)
    _q95 = np.percentile(_ent, 95)

    mo.md(f"""
    ## Unlabeled entropy summary
    ...
    High-entropy samples are candidates for round 2 active learning annotation.
    """)
    return
...
def _(mo, np, pl, unlabeled_df):
    def embedding_umap_selection_overlay():
        ...
        emb = np.array(unlabeled_df["cls_embedding"].to_list(), dtype=np.float32)[
            keep_i
        ]
        ...
        out_fpath = pathlib.Path(
            "docs/experiments/009-round2-training/results/umap-selection-overlay.png"
        )
...
@app.cell
def _(alt, mo, np, pl, results_dpath):
    def cluster_coverage():
        ...
        labels = all_unlabeled["cluster_ids"].to_numpy()
        k = int(labels.max()) + 1
    ...
    For each of {k} K-means clusters (from rank.py): what fraction of that cluster's priority beetles ended up in selected groups?
