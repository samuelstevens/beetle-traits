...
# CONFIGURATION
RENAME_LOG_PATH = pathlib.Path("./test_template_match_output/rename_log_20251112_233103.csv")
...
def validate_rename_log(log_path: pathlib.Path) -> bool:
    ...
    try:
        with open(log_path, 'r') as f:
            ...
            required_fields = {'group_image', 'old_position', 'new_position', 'old_path', 'new_path'}
            if not required_fields.issubset(reader.fieldnames):
                print(f"ERROR: Rename log missing required fields.")
...
def apply_renames(operations: list[dict], base_dir: pathlib.Path, dry_run: bool = True) -> tuple[int, int]:
    """
        ...
        for idx, (temp_full, new_full, op) in enumerate(temp_mappings, 1):
            ...
            try:
                ...
                try:
                    ...
                    print(f"    ⚠ WARNING: Could not restore temp file: {restore_error}")

    return successful, failed


def create_backup_list(operations: list[dict], output_dir: pathlib.Path, base_dir: pathlib.Path) -> pathlib.Path:
    """
...
def main():
    ...
    if CREATE_BACKUP_LIST and not DRY_RUN:
        backup_file = create_backup_list(operations, RENAME_LOG_PATH.parent, INDIVIDUAL_IMAGES_DIR)
    ...
    successful, failed = apply_renames(operations, INDIVIDUAL_IMAGES_DIR, dry_run=DRY_RUN)
    ...
    if DRY_RUN:
        ...
        if CREATE_BACKUP_LIST:
            print(f"Backup list saved for potential rollback.")
