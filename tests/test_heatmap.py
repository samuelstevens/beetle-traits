...
def test_udp_roundtrip_recovers_original_points(points_b2: np.ndarray):
    cfg = btx.objectives.heatmap.Config(image_size=256, heatmap_size=64, sigma=2.0)
    points_hm_b2 = btx.objectives.heatmap.image_to_heatmap_udp(
        jnp.asarray(points_b2, dtype=jnp.float32), cfg=cfg
    )
    points_img_b2 = btx.objectives.heatmap.heatmap_to_image_udp(points_hm_b2, cfg=cfg)
...
def test_udp_roundtrip_recovers_original_heatmap_points(points_hm_b2: np.ndarray):
    cfg = btx.objectives.heatmap.Config(image_size=256, heatmap_size=64, sigma=2.0)
    points_img_b2 = btx.objectives.heatmap.heatmap_to_image_udp(
        jnp.asarray(points_hm_b2, dtype=jnp.float32), cfg=cfg
    )
    points_hm2_b2 = btx.objectives.heatmap.image_to_heatmap_udp(points_img_b2, cfg=cfg)
...
def test_udp_maps_image_borders_to_expected_heatmap_coords():
    ...
    points_hm_b2 = btx.objectives.heatmap.image_to_heatmap_udp(points_b2, cfg=cfg)
...
def test_make_targets_returns_four_endpoint_heatmaps():
    cfg = btx.objectives.heatmap.Config(image_size=256, heatmap_size=64, sigma=1.5)
    ...
    heatmaps_chw = btx.objectives.heatmap.make_targets(points_l22, cfg=cfg)
...
def test_make_targets_channel_order_matches_width_then_length_endpoints():
    cfg = btx.objectives.heatmap.Config(image_size=256, heatmap_size=64, sigma=0.5)
    points_l22 = jnp.array(
        [
            [[1.5, 1.5], [5.5, 9.5]],
            [[13.5, 17.5], [21.5, 25.5]],
...
def test_heatmaps_to_coords_returns_line_endpoint_coordinates():
    ...
    coords_l22 = btx.objectives.heatmap.heatmaps_to_coords(logits_chw, cfg=cfg)
...
def test_heatmaps_to_coords_decodes_perfect_gaussian_logit_without_bias():
    ...
    tgt_chw = btx.objectives.heatmap.make_targets(points_l22, cfg=cfg)
    logits_chw = jnp.log(jnp.maximum(tgt_chw, cfg.eps))
    decoded_l22 = btx.objectives.heatmap.heatmaps_to_coords(logits_chw, cfg=cfg)
...
def test_heatmap_loss_zero_when_predictions_match_targets():
    ...
    loss = btx.objectives.heatmap.heatmap_loss(pred_chw, tgt_chw, loss_mask_l, cfg=cfg)
...
def test_heatmap_ce_loss_perfect_logits_equals_target_entropy():
    cfg = btx.objectives.heatmap.Config(image_size=32, heatmap_size=8, sigma=1.5)
    points_l22 = jnp.array(
        [
            [[6.0, 6.0], [10.0, 10.0]],
            [[16.0, 18.0], [24.0, 22.0]],
    ...
    loss = btx.objectives.heatmap.heatmap_ce_loss(
        pred_chw, tgt_chw, loss_mask_l, cfg=cfg
    )
    entropy_ch = -jnp.sum(
...
def test_heatmap_ce_loss_is_permutation_invariant_per_line():
    ...
    loss_direct = btx.objectives.heatmap.heatmap_ce_loss(
        pred_chw, tgt_chw, loss_mask_l, cfg=cfg
    )
    loss_swapped = btx.objectives.heatmap.heatmap_ce_loss(
...
def test_heatmap_ce_loss_has_nonzero_gradients():
    ...
    def loss_fn(pred: jax.Array) -> jax.Array:
        return btx.objectives.heatmap.heatmap_ce_loss(
            pred, tgt_chw, loss_mask_l, cfg=cfg
        )
...
def test_heatmap_ce_loss_permutation_invariant_randomized(
    ...
    tgt = btx.objectives.heatmap.make_targets(
        jnp.asarray(points_l22, dtype=jnp.float32), cfg=cfg
    )
    mask = jnp.asarray(loss_mask_l, dtype=jnp.float32)
    swapped = jnp.stack([pred[1], pred[0], pred[3], pred[2]], axis=0)
    loss_direct = btx.objectives.heatmap.heatmap_ce_loss(pred, tgt, mask, cfg=cfg)
    loss_swapped = btx.objectives.heatmap.heatmap_ce_loss(swapped, tgt, mask, cfg=cfg)
...
def test_heatmap_ce_loss_matches_manual_line_formula(
    ...
    mask = jnp.asarray(loss_mask_l, dtype=jnp.float32)
    loss = btx.objectives.heatmap.heatmap_ce_loss(pred, tgt, mask, cfg=cfg)
...
def test_channel_names_match_expected_endpoint_order():
    assert btx.objectives.heatmap.CHANNEL_NAMES == (
        "width_p0",
        "width_p1",
        "length_p0",
        "length_p1",
...
def test_get_diagnostics_flags_uniform_maps():
    ...
    max_logit_bc, entropy_bc, near_uniform_bc = btx.objectives.heatmap.get_diagnostics(
        logits_bchw, cfg=cfg
    )
